...
- **The support surface is rejected, not just gated.** A floor is depth-continuous with whatever stands on it, so region growing crawls across the whole room and the gate then grows to fit (observed 2026-08-08: a standing fan measured as a 110×86×41 cm slab of floor). Two rejections: points within 5 cm of ARKit's lowest horizontal plane (hence `planeDetection = [.horizontal]` — do not set it back to `[]`), and horizontal-normal points >12 cm below the seed. Both must *skip the BFS enqueue*, so the flood can't travel through the floor.
- The normal test is limited to points below the seed on purpose — unlimited, it would delete the top of the very table or cabinet being measured.
